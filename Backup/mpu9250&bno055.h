#pragma once
#include <Arduino.h>
#include <Wire.h>
#include <MPU9250_asukiaaa.h>
#include <Adafruit_BNO055.h>

// Configuración de multiplexor I2C y sensor
#define TCAADDR 0x70
#define I2C_KHZ 400000UL

void tcaselect(uint8_t i);
void tcadeselectAll();
bool rd8(uint8_t addr,uint8_t reg,uint8_t& val);
bool wr8(uint8_t addr,uint8_t reg,uint8_t val);
void mpu_quarantine(uint8_t a);

struct MpuReading {
  bool valid;
  float ax, ay, az;
  float gx, gy, gz;
  float mx, my, mz;
  float tempC;
};

struct BnoReading {
  bool valid;
  float eulerX, eulerY, eulerZ;
  float accelX, accelY, accelZ;
  float gyroX, gyroY, gyroZ;
};

struct SensorFrame {
  unsigned long timestamp_ms;
  MpuReading mpu;
  BnoReading bno;
};

bool read_mpu_on_channel(uint8_t ch, MpuReading* out = nullptr, bool emitSerial = true);
bool read_bno_on_channel(uint8_t ch, BnoReading* out = nullptr, bool emitSerial = true);
void sensores_init();
